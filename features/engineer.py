...
def engineer_features(claim: dict) -> dict:
    ...
    tariff  = float(
        claim.get("approved_tariff", 0) or
        VALID_PROCEDURE_TARIFFS.get(claim.get("procedure_code", ""), 1)
    )
    ...
    member_claim_frequency   = int(claim.get("member_claim_frequency", 1))
    provider_claim_frequency = int(claim.get("provider_claim_frequency", 1))
    is_duplicate             = int(claim.get("is_duplicate", 0))
    member_age               = int(claim.get("member_age", 35))
