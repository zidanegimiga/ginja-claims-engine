...
The constraint that shaped every decision: this was a case study to be completed in one day, demo-recorded, and presented in a technical interview. That meant making deliberate choices about where to invest time and where to acknowledge gaps honestly.

The approach: build production-grade infrastructure (auth, logging, drift detection, model registry, tests) around a model trained on synthetic data, then be transparent about what changes before real deployment. A clean architecture with honest documentation is more valuable to an interviewer than a model with impressive metrics built on shaky foundations.
...
**Three critical insights from the forms analysis:**

First, a claim in this system is not one document. It is a pair: the patient submits a claim form, the hospital issues an invoice. Both belong to the same claim event. Our data model had to reflect this from the start.
...
Three reasons. First, XGBoost works well on tabular data with engineered features, which is exactly what structured claims data is. Neural networks on tabular data rarely outperform gradient boosting without significantly more data. Second, XGBoost is fast at inference, which matters for real-time adjudication. Third, SHAP integration with XGBoost is mature and well-supported.
...
**Features engineered from raw claim fields:**
...
From the ECM Health Insurance claim form: claim ID, member ID, patient name, amount (4,300 KES), date of service, location, provider type, patient age. Additionally, Gemini noted a date discrepancy between the treatment date (2025) and the signature date (2026), without being explicitly instructed to check for this. That is intelligent extraction.
...
Name matching cannot be exact. Names appear in different word orders on different forms. "Sharma Siddharth" and "Siddharth Sharma" are the same person. The initial "S. Sharma" and "Siddharth Sharma" are the same person.
...
The intuition: if the model was trained when the average claimed amount was 5,000 KES, and recent claims average 15,000 KES, the model's learned relationship between amount deviation and fraud risk may no longer be calibrated correctly. The model needs retraining on current data.
...
**XGBoost and OpenMP on Apple M4:**

XGBoost requires OpenMP for multi-threaded training. On Apple Silicon Macs, OpenMP is not installed by default. The symptom was an import error mentioning `libgomp`. The fix: `brew install libomp`.

**The `google-generativeai` deprecation:**

The original Gemini integration used the `google-generativeai` package. This package was deprecated and replaced with `google-genai`. The new package has a different API surface. The provider implementation was rewritten against the new package.

**`int(None)` crash in feature engineering:**

When `approved_tariff` or `member_age` was missing from a claim, the feature engineering function crashed attempting to compute numerical features. The fix: replace direct type conversions with `or default` patterns that substitute zero or a reasonable default when values are absent.

**`str.startswith()` on None in rules engine:**

The rules engine called `.startswith()` on provider type and diagnosis code fields without null-checking first. When these fields were absent (common in partially extracted PDF claims), the function crashed. The fix: add `if field and field.startswith(...)` null guards throughout the rules engine.

**Reserved logging field name collision:**

Python's logging system reserves the field name `name` for the logger's own name. Passing `name` as a key in the `extra` dictionary raises a `KeyError`. The fix: rename the field to `key_name` in all logger calls in `api/auth.py`.

**FastAPI dependency order for authentication:**

As described in the API Design section, calling authentication manually inside a route handler allows FastAPI to validate the request body before authentication runs. The fix: declare authentication as a `Depends()` parameter to guarantee it runs first.

**Python parameter ordering with defaults:**

FastAPI route functions that mix parameters with defaults (`Depends()`, `Query()`) and parameters without defaults (`Request`) must place parameters without defaults before parameters with defaults. Violating this order produces a `SyntaxError: parameter without a default follows parameter with a default`.
...
**Data:**
- Train on real labelled claims data from actual Eden Care submissions
- Build a real provider risk database from historical claim patterns
- Integrate against a live policy/coverage database for member eligibility checks
- Replace the curated ICD-10/CPT code subsets with full code databases

**Infrastructure:**
- Redis-backed rate limiting for horizontal scaling
- Dedicated log aggregation (Datadog or ELK) separate from MongoDB
- Secrets management via AWS Secrets Manager or HashiCorp Vault rather than .env files
- HTTPS with TLS termination at the load balancer
- Database connection pooling for high-concurrency workloads

**Security:**
- JWT for end-user authentication (claims officers logging in via the frontend)
- API keys only for service-to-service (frontend backend, batch processors)
- Regular automated key rotation
- Penetration testing before launch given the sensitivity of healthcare data

**Model:**
- Retrain on real data after 3 to 6 months of production claims
- Add a human review queue with feedback loop: reviewers' accept/reject decisions become training labels
- Monitor feature drift monthly and trigger automated retraining when PSI exceeds threshold
- A/B test new model versions on a small percentage of traffic before full rollout

**Compliance:**
- Data protection impact assessment under Kenya's Data Protection Act 2019
- Rwanda's data protection regulations (Law No. 058/2021)
- Audit trail for every adjudication decision and every model retrain
- Data retention and deletion policies for claims records
